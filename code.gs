// =====================================================================
// CytoFlow v1.3.8 AI Core - Multi-Database Architecture (SaaS Ready)
// =====================================================================
...
function doGet() {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('CytoFlow 2026 (v1.3.8 AI Core)')
...
// --- API: STICKER CONFIG (v1.3.8 AI Core) ---
// ดึงค่าการตั้งค่าสติ๊กเกอร์ (อัปเดตเพิ่มโครงสร้างแบบ JSON สำหรับ WYSIWYG)
function apiGetStickerConfig() {
  try {
    const sysSS = SpreadsheetApp.openById(DB_FILES.SYSTEM);
    let sheet = sysSS.getSheetByName('Sticker_Config');
    
    // Default Configuration + Layout JSON
    let config = {
      width: 50,           // ความกว้าง (มิลลิเมตร)
      height: 25,          // ความสูง (มิลลิเมตร)
      autoPrintCount: 2,   // จำนวนแผ่นที่สั่งปริ้นอัตโนมัติ
      manualPrintCount: 1, // จำนวนแผ่นเมื่อกดปุ่มปริ้นเอง
      showBarcode: true,   // แสดงบาร์โค้ดหรือไม่
      barcodeHeight: 30,   // ความสูงของเส้นบาร์โค้ด (px)
      barcodeWidth: 1.5,   // NEW: ความกว้าง/สเกลของเส้นบาร์โค้ด (เผื่อจับหมุนแนวตั้งแล้วยาวเกิน)
      // โครงสร้างพิกัด X,Y และการหมุน (เก็บเป็น JSON เพื่อให้ Database ไม่รก)
      layoutJSON: JSON.stringify({
        cyto: { x: 25, y: 4, size: 12, rot: 0, align: 'center', visible: true, bold: true },
        name: { x: 25, y: 9, size: 9, rot: 0, align: 'center', visible: true, bold: false },
        age:  { x: 25, y: 13, size: 9, rot: 0, align: 'center', visible: true, bold: false },
        spec: { x: 25, y: 17, size: 9, rot: 0, align: 'center', visible: true, bold: false },
        bar:  { x: 25, y: 22, rot: 0, align: 'center', visible: true, width: 1.5 }
      })
    };

    if (!sheet) {
      // สร้างแท็บและเขียนค่าเริ่มต้นหากยังไม่มี
      sheet = sysSS.insertSheet('Sticker_Config');
      sheet.appendRow(['Key', 'Value', 'Description']);
      sheet.getRange("A1:C1").setFontWeight("bold").setBackground("#f1f5f9");
      
      const descriptions = {
        width: "ความกว้างของสติ๊กเกอร์ (mm)",
        height: "ความสูงของสติ๊กเกอร์ (mm)",
        autoPrintCount: "จำนวนแผ่นที่จะพิมพ์อัตโนมัติเมื่อลงทะเบียนเสร็จ",
        manualPrintCount: "จำนวนแผ่นที่จะพิมพ์เมื่อกดปุ่มปริ้นจากหน้าจอ",
        showBarcode: "แสดงบาร์โค้ด (true/false)",
        barcodeHeight: "ความสูงของเส้นบาร์โค้ด (px)",
        barcodeWidth: "สเกลความกว้าง/ความยาวของบาร์โค้ด", // NEW
        layoutJSON: "พิกัด X/Y, ขนาด, การหมุน ของแต่ละองค์ประกอบ (ห้ามแก้ไขด้วยมือ)"
      };

      for (let key in config) {
        sheet.appendRow([key, config[key], descriptions[key]]);
      }
    ...
    return { status: 'error', message: 'Get Sticker Config Error: ' + e.message }; 
  }
}

// สำหรับ Admin บันทึกค่าการตั้งค่าสติ๊กเกอร์ใหม่
function apiSaveStickerConfig(newConfig, username) {
  ...
  try {
    ...
    // อัปเดตข้อมูล (หากมีการเพิ่ม Key ใหม่ผ่านระบบ Layout ให้เพิ่มลง Sheet ด้วย)
    for (let key in newConfig) {
      ...
      // ถ้าไม่มี Key ในฐานข้อมูลเดิม (เผื่ออัปเดตเวอร์ชั่น)
      if (!found) {
        sheet.appendRow([key, newConfig[key], "Auto-generated field"]);
      }
    }
      ...
      for(var i=0; i<data.length; i++) {
        if(data[i][0] && data[i][0].toString().trim() !== "") {
          res.push({ 
            ...
            sex: (data[i][1] ? data[i][1].toString().trim() : 'หญิง')
          });
        }
      }
      return res;
...
function apiRegisterSample(form, year, username) {
  ...
  try {
    ...
    // Server-side Sanitization: จำกัดให้อายุเป็นตัวเลขและไม่เกิน 3 หลัก (Req 5)
